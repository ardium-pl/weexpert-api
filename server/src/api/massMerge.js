...
export function reconcileTransactions(bankTransactions, invoices) {
    ...
    bankTransactions.forEach(transaction => {
        ...
        if (matchResult.matchFound) {
            removeInvoiceFromRemaining(remainingInvoices, matchResult.invoiceData.invoice);
...
function findMatchingInvoice(transaction, invoices) {
    ...
    const potentialMatches = invoices
        .filter(invoice => transaction.currency === invoice.currency && nonFinancialVendor(transaction.contractor))
        .map(invoice => ({
            invoice, 
    ...
    if (!bestMatch || bestMatch.checksPassed < checkSweetSpot) {
        return { matchFound: false, invoiceData: { nonNegotiableCriteriaNotMet: true } };
    }
...
function performAdditionalChecks(transaction, invoice) {
    ...
    const checksPassed = Object.values(checkDetails).reduce((count, passed) => count + passed, 0);
    return { checksPassed, checkDetails };
}

function contractorNameCheck(transactionContractor, invoiceContractor) {
    const sanitizedTransactionContractor = transactionContractor.replace(/[1\/,""]/g, "").toLowerCase();
    const [sanitizedInvoiceContractor] = invoiceContractor.replace(/[,"]/g, "").toLowerCase().split(" ");
